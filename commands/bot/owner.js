            ...
            if(client.config.owner.includes(message.author.id)) {
                ...
                if (args[0] === "add") {
                    ...
                    if (!member) return message.channel.send(`Aucun membre trouvé pour \`${args[1]|| " "}\``)
                           if (db.get(`ownermd_${client.user.id}_${member.id}`) === true) { return message.channel.send(`${member.username} est déjà owner`)}
                        ...
                        if(args[1]){
                            ...
                            if (!member) return message.channel.send(`Aucun membre trouvé pour \`${args[1]|| " "}\``)
                            if (db.get(`ownermd_${client.user.id}_${member.id}`) === null) return message.channel.send(`${member.username} n'est pas owner`)
